import { WORK_MODES } from '../constants.js';

export const jobs = [
  {
    id: 'turing-data-scientist-ai-training',
    title: 'Data Scientist / Analyst — AI Training',
    company: 'Turing',
    category: 'Analytics / Data Science',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Global contractor pool; source indexes India-specific AI-training roles and describes openings as available from any location.',
    confidence: 0.92,
    score: 3.8,
    pay: '$50–$80/hour listed by source aggregator',
    sourceUrl: 'https://aitrainer.work/jobs/india/',
    companyUrl: 'https://www.turing.com/jobs',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'Large AI-talent marketplace with repeat global hiring for coding, analytics, and LLM evaluation projects.',
      'Contractor-style work can vary by project availability, so applicants should confirm payment terms before onboarding.',
      'Included because India-targeted listings and broad global contractor sourcing meet the 90% eligibility threshold.'
    ],
    tags: ['AI training', 'SQL', 'Python', 'Data analysis']
  },
  {
    id: 'turing-mle-bench-ml-engineer',
    title: 'MLE Bench — ML Engineers',
    company: 'Turing',
    category: 'AI Development',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Open global AI-training marketplace; India page lists role with no local office requirement in source summary.',
    confidence: 0.91,
    score: 3.8,
    pay: '$40–$70/hour listed by source aggregator',
    sourceUrl: 'https://aitrainer.work/jobs/india/',
    companyUrl: 'https://www.turing.com/jobs',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'Best fit for experienced ML engineers comfortable with project-based expert review work.',
      'Score is moderated because marketplace roles can have inconsistent project continuity.',
      'Applicant should verify whether the contract is direct with Turing or a client-specific program.'
    ],
    tags: ['ML engineering', 'LLM evaluation', 'Python']
  },
  {
    id: 'mindrift-statistics-python-ai-trainer',
    title: 'Statistics & Python Expert — Freelance AI Trainer',
    company: 'Mindrift',
    category: 'Analytics / Data Science',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Remote AI-training role indexed as remote without country restriction in the current source list.',
    confidence: 0.9,
    score: 3.5,
    pay: 'Source-listed annualized estimate varies by board',
    sourceUrl: 'https://remotejobs.org/remote-jobs/ai-training-jobs',
    companyUrl: 'https://mindrift.ai/careers',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'Freelance AI-training model is suitable for global specialists but should be treated as variable-hours contract work.',
      'No country restriction appeared in the indexed listing title/summary used for inclusion.',
      'Applicants should independently check payment method availability for India before accepting tasks.'
    ],
    tags: ['Statistics', 'Python', 'Freelance', 'AI training']
  },
  {
    id: 'toloka-first-person-video-ai-trainer',
    title: 'First-Person Video AI Trainer',
    company: 'Toloka AI',
    category: 'AI Training',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Source summary explicitly labels the role worldwide, making India eligibility likely above the confidence bar.',
    confidence: 0.93,
    score: 3.6,
    pay: 'Source-listed project estimate; confirm task-by-task pay',
    sourceUrl: 'https://remotejobs.org/remote-jobs/ai-training-jobs',
    companyUrl: 'https://toloka.ai/tolokers/',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'Established crowdsourcing and AI-data platform with large global contributor base.',
      'Task marketplaces can include uneven earning potential; review task rates and privacy terms carefully.',
      'Included only where the listing is worldwide or otherwise remote without a country lock.'
    ],
    tags: ['Data annotation', 'Video', 'AI training']
  },
  {
    id: 'clouddevs-fullstack-backend-llm',
    title: 'Full-stack / Backend Developers — LLM Focused',
    company: 'CloudDevs',
    category: 'AI Development',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Indexed as "Anywhere in the World"; suitable for Indian nationals working remotely when client contract permits.',
    confidence: 0.91,
    score: 3.7,
    pay: 'Contract rate varies by client engagement',
    sourceUrl: 'https://remotejobs.org/remote-jobs/ai-training-jobs',
    companyUrl: 'https://clouddevs.com/jobs/',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'Talent marketplace model with remote client projects and screening before placement.',
      'Applicants should confirm tax paperwork, payment rails, and client location restrictions before interviews.',
      'Included because the listing source says anywhere in the world and the role aligns with AI/LLM development.'
    ],
    tags: ['LLM', 'Backend', 'Full-stack', 'Contract']
  },
  {
    id: 'tefl-online-english-teacher-celta',
    title: 'Online English Teacher — CELTA / CertTESOL',
    company: 'TEFL.com listed employer',
    category: 'ESL Tutoring',
    workMode: WORK_MODES.WORLDWIDE_REMOTE,
    eligibility: 'Source summary names India among accepted remote teacher locations and describes worldwide online learners.',
    confidence: 0.9,
    score: 3.4,
    pay: 'Check source listing for current rate',
    sourceUrl: 'https://www.tefl.com/job-seeker/jobpage.html?countryId=&jobId=219037',
    companyUrl: 'https://www.tefl.com/job-seeker/',
    verifiedOn: '2026-06-09',
    credibilityNotes: [
      'ESL listings can change quickly; treat this as a source-backed lead, not an endorsement of every employer on the board.',
      'Included because the current search summary explicitly references India and fully remote teaching.',
      'Applicants should check teacher classification, cancellation policy, and student-allocation rules.'
    ],
    tags: ['ESL', 'CELTA', 'Remote teaching']
  }
];
